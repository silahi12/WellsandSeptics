package pomFramework.testsPom.EtsTests;
...
public class Mvp_HappyPath_ApplyOnESC_VerifyOnETS extends BaseTestPom {
    ...
    //Permit info
    private static final String PERMIT_TYPE = "Air Quality Standard Permit to Construct";
    private static final String PERMIT_DETAIL_TYPE = "Processing or Manufacturing Equipment";
    private static final String PERMIT_ACTION_TYPE = "Standard Permit to Construct";

    //Files info
    //Download files
    private static final List<String> DOWNLOAD_FILES = List.of("Application Form for Processing or Manufacturing Equipment File",
                                                                "Application Form for Air Toxics Summary Sheet (if applicable) File"
                                                                ,"Emission Point Data File",
                                                                "Application for Control Device (if applicable) File");
    private static final List<String> DOWNLOAD_FILES_PDF_NAMES = List.of("Air Quality Standard Permit to Construct - Processing and Manufacturing - Form 5 - ARA.pdf",
                                                                        "Air Quality Standard Permit to Construct - Toxic Air Pollutant Emissions Summary and Compliance Demonstration - Form 5T - ARA.pdf",
                                                                        "Air Quality Standard Permit to Construct - Emissions Point Data - Form 5EP - ARA.pdf",
                                                                        "Air Quality Standard Permit to Construct - Emission Control Equipment - Form 6 -ARA.pdf");
    //Upload Files
    private static final String REQUIRED_UPLOAD_FILE = "Permit Application Package";

    //Fees
    private static final String TOTAL_FEE = "To Be Determined";

    @Test(description = "Happy path test for applying Air Quality Standard Permit to Construct - Processing or Manufacturing Equipment - Standard Permit to Construct")
    public void mvpHappyPathAirQualityStandardPermitToConstruct() throws InterruptedException {
        ...
        EtsLoginPage etsLoginPage = new EtsLoginPage();
        EtsOnlineStagingPage etsOnlineStagingPage = new EtsOnlineStagingPage();
        EtsIntDocPage etsIntDocPage = new EtsIntDocPage();
        ...
        String localFileName2 = "images.jpg"; // This file should be in src/test/resources/resourcesPOM/testFiles/
        String localExactFileName2 = "images";
        // Construct the absolute path to your test file
        ...
        mvpUploadPage.uploadAdditionalFileMvp(filePath2);
        ...
        mvpDashboardPage.waitUntilSuccessAlertDisappear();
        String applicationID = mvpDashboardPage.getLatestApplicationID();

        mvpDashboardPage.clickMvpLogout();

        etsLoginPage.loginToEts();
        etsLoginPage.goToOnlineStaging();
        etsOnlineStagingPage.waitForOnlineStagingPage();

        //search for app id
        etsOnlineStagingPage.searchAndViewApp(applicationID);

        etsIntDocPage.waitForIntDocPage();
        etsIntDocPage.clickFileDocMngTab();
        etsIntDocPage.verifyFileNameInTable(REQUIRED_UPLOAD_FILE);
        etsIntDocPage.verifyFileNameInTable(localExactFileName2);

        //logout
        etsLoginPage.logoutEts();
